...
# Usage: python3 getSNPflankingregions.py <input_file> <input_fasta> <flankingregionupstreamlength> <flankingregiondownstreamlength> OR only supply one length that will be used for both up and downstream
...
def make_flanks_from_bed(bed_input, flank_up, flank_down):
    bed_lines = []
    for chrom, pos_1, pos_2 in bed_input:
        start = pos_1 - int(flank_up) - 1  # 0-based BED format
        end = pos_1 + int(flank_down)
        name = f"{chrom}:{start}-{end}"
        bed_lines.append(f"{chrom}\t{start}\t{end}\t{name}")
...
def main():
    if len(sys.argv) not in [4, 5]:
        print("Usage: python3 getSNPgenomeannotations.py <input_file> <input_fasta> <flankingregionupstreamlength> <flankingregiondownstreamlength>")
        print("OR")
        print("Usage: python3 getSNPgenomeannotations.py <input_file> <input_fasta> <flankingregionlength>")
        sys.exit(1)
    ...
    if not input_file.endswith(".vcf", ".bed"):
        print("Error: Input file must have either .vcf or .bed extension.")
        sys.exit(1)
    ...
    if input_file.endswith(".vcf"):
        ...
        print("[INFO] Creating BED entries for flanking regions...")
        bed = make_flanks_from_bed(bed_input, flank_up, flank_down)
        print("[INFO] Extracting sequences using pybedtools...")
        output_fasta = extract_sequences(bed, input_fasta, snps, flank_up, flank_down)
